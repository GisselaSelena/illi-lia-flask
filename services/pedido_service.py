...
def crear_pedido(id_usuario, id_producto, cantidad, metodo_pago='contra entrega', comprobante=None):
    ...
    cursor.execute(
        """INSERT INTO pedidos (id_usuario, id_producto, cantidad, total, estado, metodo_pago, comprobante)
           VALUES (%s, %s, %s, %s, 'pendiente', %s, %s)""",
        (id_usuario, id_producto, cantidad, total, metodo_pago, comprobante)
    )
...
def obtener_todos_pedidos():
    ...
    cursor = conexion.cursor()
    cursor.execute("""
        SELECT p.id_pedido, u.nombre, pr.nombre, p.cantidad, p.total, p.estado, p.metodo_pago, p.comprobante, p.fecha
        FROM pedidos p
